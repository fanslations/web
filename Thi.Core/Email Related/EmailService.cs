...
namespace Thi.Core
{
	...
	public class EmailService : IEmailService
	{
		...
		private readonly string _mSmtpServers = ConfigurationManager.AppSettings["SmtpServers"];
		...
		{
			if (!string.IsNullOrEmpty(_mSmtpServers))
		...
		ISmtpServer SetSmtpServer(List<string> smtpServers)
		{
			...
			var smtpServer = smtpServers[0].ToLower();
			smtpServers.Remove(smtpServer);
			switch (smtpServer.Trim())
			{
				case "gmail":
				...
				default:
					return new DefaultSmtpServer("Default");
			}
		}
